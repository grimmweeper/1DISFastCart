...
#########################################################################################################
    ...
    while user is not None:
        ...
        if (hx.get_weight_mean(3) - running_weight >= 50) or (running_weight - hx.get_weight_mean(3) >= 50):
            ...
            illop_count += 1
            print("ILLOPPPPPPPPP COUNT" + str(illop_count))
            if illop_count >= 5:
                trolleys.document(trolley_id).set({f'illop': True}, merge=True)

        # else reser illop_count
        else:
        ...
        while illop:
            ...
            # if normal alr, get out of illop
            if not ((hx.get_weight_mean(3) - running_weight >= 50) or (running_weight - hx.get_weight_mean(3) >= 50)):
                trolleys.document(trolley_id).set({f'illop': False}, merge=True)
